...
def _runtime_python_root():
    ...
    root = (_RELEASE_ROOT / "runtime" / python_path).resolve()
    runtime_root = (_RELEASE_ROOT / "runtime").resolve()
    try:
...
def process(content, attributes, parameters):
    ...
    try:
        ...
        invoke_kwargs = _build_kwargs(_PLAN.get("arguments", {}), content, attributes, parameters)
        ...
        if payload_value is None:
            output_content = content if output_spec.get("none_policy") == "keep-original" else b""
        else:
