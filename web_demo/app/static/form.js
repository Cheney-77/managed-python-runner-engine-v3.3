  ...
  function editor(schema, config = {}) {
    ...
    if ((enumValues && enumValues.length) ||
      ...
      const values = enumValues && allowed
        ...
        : (allowed || enumValues);
      if (!values?.length) {
        block.append(node("p", "editor-error", "Schema 可选值与 Analyze allowedSources 不一致。"));
        return {
          element: block,
          read: () => fail(title, "Schema enum 与 allowedSources 无交集"),
        };
      }
    ...
    if (kind === "object") {
      ...
      for (const [key, definition] of Object.entries(props)) {
        ...
        heading.append(node("span", "", `${label}${required.has(key) ? " *" : ""}`));
        ...
        const value = Object.prototype.hasOwnProperty.call(init, key)
          ? init[key] : S.template(definition, doc);
