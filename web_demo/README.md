# Managed Python v3.3 — Chrome 风格 Operator Console（绑定交互增强版）
...
这版沿用上一版 Chrome 风格视觉，进一步优化了参数来源和输出格式：

- 每个构造参数、调用参数先选择 **输入内容 / 输入属性 / 算子参数 / 固定值**，
  选择后自动显示对应字段，不再把所有选项并排堆在同一表单。
- 可识别 `bindingSource`、`binding_source`、`source` 等来源字段，
  并保留实际 OpenAPI 的字段键名和服务端要求的枚举值。
- 对 OpenAPI 的 discriminated `oneOf` / `anyOf` 分支按实际来源切换；
  扁平 Schema 则依据明确的字段名称进行分组，未能确定含义的字段留在
  **更多设置**，不会伪造 API 参数。
- 来源切换会清除**提交请求**中不属于当前来源的字段；已完成的来源配置
  在切回时可恢复。推荐配置仅在字段和来源能被当前 Schema 确认时应用。
- 固定值若是自由 JSON 类型，提供文本、数字、布尔、对象/数组、null 的
  类型选择；若服务端 Schema 明确要求 string/int 等，则使用对应的真实类型控件。
- **修正旧版的枚举模板问题：**如果服务器只声明 `enum`、没有提供
  `default` / `const`，页面现在要求用户明确选择，不再自动选列表第一个值。
  旧版代码曾使用 `enum[0]` 作为模板初始值，如果输出 codec 的第一项恰好是
  `bytes`，可能让用户无意间创建 bytes 输出契约（是否发生于具体算子仍需核对已保存契约）。
- 输出选择 bytes 时展示类型提示；若返回注解声明为 dict/list 等，提醒确认
  是否匹配，但不会自动把 bytes 改成 JSON、也不会偷偷转换用户返回值。
  详见 [`docs/output-codec-investigation.md`](docs/output-codec-investigation.md)。
...
必填参数的绑定默认启用且不可关闭；可选参数可根据需要启用。
绑定来源先由实际 `allowedSources` 提供，并与 OpenAPI 来源字段的枚举取交集；
选定来源后，带有条件分支的 Schema 切到对应分支，
扁平 Schema 则隐藏已知不适用的字段。若模型不包含可辨识的来源字段，
界面显示不可配置提示，不会乱造 Contract。可查看 Analyze 建议；建议的字段如果能
...
此浏览器测试还覆盖 Callable 搜索、参数配置进度、Runner Profile 和 JSON 同步、
发布对话框取消/确认、390px 手机布局无整体横向溢出，
**两组参数的来源切换与字段剔除、切回恢复，以及 bytes 输出提示**等交互。
...
为了让表单从“Schema 通用编辑器”升级为业务化配置界面，
最好补充以下模型（不阻塞现有工程通过实际 OpenAPI 工作）：
