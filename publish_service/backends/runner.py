...
COMPILER_VERSION = "runner-contract-v1"
...
def write_runner_release_files(
    ...
    *,
    ...
    (root / "__dsc_entry__.py").write_text(generate_runner_adapter(plan), encoding="utf-8")
    (root / "operator.yaml").write_text(
        yaml.safe_dump(contract.runner_manifest.model_dump(mode="json"), sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    ...
    publish_info = {
        ...
        "execution_plan_sha256": hashlib.sha256(canonical_json(plan.model_dump(mode="json"))).hexdigest(),
