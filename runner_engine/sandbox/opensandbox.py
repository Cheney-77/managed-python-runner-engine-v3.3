...
# These are transport/readiness guardrails, not operator execution budgets.
...
class OpenSandboxBackend:
    ...
    The secure runtime is selected by OpenSandbox server configuration. For this deployment
    we intentionally do not send secureAccess or networkPolicy in create requests.
    """
    ...
    def _cluster(self, name: str) -> dict[str, str]:
        try:
            ...
            raise SandboxError("UNKNOWN_SANDBOX_CLUSTER", f"no OpenSandbox cluster named {name}") from exc
    ...
    def _sandbox_id(value: dict) -> str:
        ...
        raise SandboxError("OPENSANDBOX_PROTOCOL", "create response did not contain a sandbox id")
    ...
    def create(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            "resourceLimits": {"cpu": policy.cpu, "memory": policy.memory},
            "env": {
        ...
        # - secureAccess is not sent for Docker/direct ingress.
        # - networkPolicy is not sent for gVisor because OpenSandbox's egress sidecar
        #   and gVisor netstack are not compatible in this deployment.
        # Enforce egress policy outside the sandbox (host/CNI/firewall) when required.
        created = self._request(cluster, "POST", "/v1/sandboxes", payload)
        sandbox_id = self._sandbox_id(created)

        try:
            ready_deadline = time.monotonic() + SANDBOX_READY_WAIT_SECONDS
            while time.monotonic() < ready_deadline:
                status = self._request(cluster, "GET", f"/v1/sandboxes/{urllib.parse.quote(sandbox_id)}")
                state = self._state(status)
                if state in {"RUNNING", "READY", "STARTED"}:
                    break
                if state in {"FAILED", "STOPPED", "ERROR", "TERMINATED"}:
                    raise SandboxError("SANDBOX_START_FAILED", f"sandbox entered state {state}", retryable=True)
                time.sleep(0.2)
            else:
                raise SandboxError("SANDBOX_START_TIMEOUT", "sandbox did not become ready", retryable=True)

            endpoint_data = self._request(
                cluster,
                "GET",
                f"/v1/sandboxes/{urllib.parse.quote(sandbox_id)}/endpoints/{self.agent_port}",
            )
            endpoint = endpoint_data.get("endpoint") or endpoint_data.get("url")
            if not endpoint:
                raise SandboxError("SANDBOX_ENDPOINT_MISSING", "OpenSandbox endpoint response had no endpoint")
            ...
            worker = Worker(
                ...
                metadata={"endpoint_headers": dict(endpoint_data.get("headers") or {})},
            )
            ...
            raise SandboxError("AGENT_START_TIMEOUT", detail, retryable=True)
        except Exception:
            try:
                self._request(
                    cluster,
    ...
    def renew(self, worker: Worker, *, orphan_ttl_seconds: int) -> None:
        expires_at = datetime.now(timezone.utc) + timedelta(seconds=orphan_ttl_seconds)
        self._request(
            worker.backend_group,
            "POST",
            f"/v1/sandboxes/{urllib.parse.quote(worker.id)}/renew-expiration",
    ...
    def cleanup_managed(self) -> int:
        ...
        for cluster in self.clusters:
            try:
                response = self._request(cluster, "GET", "/v1/sandboxes?page=1&pageSize=100")
            except Exception:
                continue

            items = response if isinstance(response, list) else response.get("items") or response.get("sandboxes") or []
            for item in items:
