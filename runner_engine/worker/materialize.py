...
class ReleaseMaterializer:
    ...
    def install(self, release_id: str, expected_sha256: str, artifact: bytes) -> Path:
            ...
            with tarfile.open(fileobj=io.BytesIO(artifact), mode="r:*") as tar:
                for member in tar.getmembers():
                    ...
                    if member.issym() or member.islnk() or member.isdev() or member.isfifo():
                        raise ValueError(f"special/link entry is forbidden: {member.name}")
                    ...
                    if member.isfile():
                        ...
                        if total > self.max_expanded_bytes:
                            raise ValueError("artifact expands beyond configured limit")

                    destination = (staging / pure).resolve()
                    if staging.resolve() not in destination.parents and destination != staging.resolve():
                        raise ValueError(f"path escapes staging directory: {member.name}")
