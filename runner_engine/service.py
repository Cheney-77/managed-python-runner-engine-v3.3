...
def _filter_attributes(values: dict[str, str], allowed: tuple[str, ...]) -> dict[str, str]:
    if not allowed:
        return {}
    allow = set(allowed)
    return {key: value for key, value in values.items() if key in allow}


def _validate_attributes(values: dict[str, str]) -> None:
    if len(values) > MAX_ATTRIBUTES:
        raise RunnerError("TOO_MANY_ATTRIBUTES", "attribute count exceeds limit")

    total = 0
    for key, value in values.items():
        if not isinstance(key, str) or not isinstance(value, str):
            raise RunnerError("INVALID_ATTRIBUTES", "attributes must be dict[str, str]")
        total += len(key.encode("utf-8")) + len(value.encode("utf-8"))

    if total > MAX_ATTRIBUTE_BYTES:
        raise RunnerError("ATTRIBUTES_TOO_LARGE", "attributes exceed byte limit")
...
class RunnerService:
    """Main application service. Execution, idempotency and lease semantics live here."""
    ...
    def startup(self) -> int:
        ...
        if abandoned:
            logger.warning("marked %d stale RUNNING executions as ABANDONED", abandoned)
    ...
    def start_background(self, *, interval_seconds: int = 30) -> None:
        ...
        self._reaper_thread = threading.Thread(target=loop, name="managed-python-reaper", daemon=True)
    ...
    def acquire_lease(
        ...
        *,
        ...
        if release.profile not in self.policies:
            raise RunnerError("UNKNOWN_SECURITY_POLICY", f"release uses unknown profile {release.profile}")
        return self.db.create_lease(tenant_id, project_id, processor_id, release_id, self.lease_ttl_ms)

    def get_lease(self, *, tenant_id: str, lease_id: str) -> Lease:
        return self.db.require_lease(lease_id, tenant_id=tenant_id)

    def renew_lease(self, *, tenant_id: str, lease_id: str) -> Lease:
        return self.db.renew_lease(lease_id, tenant_id=tenant_id, ttl_ms=self.lease_ttl_ms)

    def release_lease(self, *, tenant_id: str, lease_id: str) -> None:
        self.db.delete_lease(lease_id, tenant_id=tenant_id)

    def invoke(self, request: RunRequest) -> RunResult:
        if len(request.content) > MAX_INLINE_BYTES:
            raise RunnerError("INLINE_CONTENT_TOO_LARGE", "inline content exceeds 8 MiB")
        if not request.idempotency_key:
            raise RunnerError("IDEMPOTENCY_REQUIRED", "idempotency_key is required")
        ...
        try:
            ...
            raise RunnerError("UNKNOWN_SECURITY_POLICY", f"unknown profile {release.profile}") from exc

        safe_attributes = _filter_attributes(request.attributes, release.input_attributes)
        ...
        safe_request = RunRequest(
            ...
            timeout_ms=min(max(1, request.timeout_ms), policy.max_timeout_ms),
        )
        fingerprint = _request_fingerprint(safe_request)

        with self._active_lock:
            if request.invocation_id in self._active:
                raise RunnerError("INVOCATION_ID_IN_USE", "invocation id is already active")
        ...
        run_id = claim.run_id
        if run_id is None:
            raise RunnerError("RUN_STATE_INVALID", "new execution did not receive a run id", retryable=True)

        worker = None
        run_finished = False

        try:
            worker = self.pool.acquire(request.tenant_id, release, policy)

            with self._active_lock:
                if request.invocation_id in self._active:
                    self.pool.invalidate(worker)
                    worker = None
                    raise RunnerError("INVOCATION_ID_IN_USE", "invocation id became active concurrently")
            ...
            result = self.pool.backend.run(worker, release, safe_request, timeout_ms=safe_request.timeout_ms)

            safe_output_attributes = _filter_attributes(result.attributes, release.output_attributes)
            _validate_attributes(safe_output_attributes)
            if len(result.content) > MAX_INLINE_BYTES:
                raise RunnerError("OUTPUT_TOO_LARGE", "operator output exceeds 8 MiB")
            ...
            # Persist the terminal execution result before returning the worker to the pool.
            # retryable=True is still recorded in runs, but its idempotency cache expires immediately.
            self.db.finish_run(
                run_id=run_id,
                key=request.idempotency_key,
                tenant_id=request.tenant_id,
            ...
            try:
                if result.status == "SUCCEEDED" or result.error_code in {
                    "TIMEOUT",
                    "USER_EXCEPTION",
                    "USER_PROCESS_EXITED",
                    "OUTPUT_LIMIT",
                ...
                logger.exception("worker cleanup failed after terminal result; result remains authoritative")
            worker = None
            ...
            if worker is not None:
                try:
                    ...
                    logger.exception("failed to invalidate worker after infrastructure error")
            ...
            raise RunnerError("RUNNER_INFRASTRUCTURE_ERROR", str(exc), retryable=True) from exc

        finally:
            with self._active_lock:
                self._active.pop(request.invocation_id, None)

    def cancel(self, *, tenant_id: str, lease_id: str, invocation_id: str) -> bool:
        self.db.require_lease(lease_id, tenant_id=tenant_id)
        ...
        with self._active_lock:
            ...
            if active.tenant_id != tenant_id or active.lease_id != lease_id:
                raise RunnerError("CANCEL_FORBIDDEN", "invocation does not belong to this tenant/lease")
            self._active.pop(invocation_id, None)

        self.pool.invalidate(active.worker)
        return True
